...
# 前端传递的参数: 地址的id(addr_id) 支付方式(pay_method), 用户要购买的商品id字符串(sku_ids)
# mysql事务,一组sql操作，要么都成功，要么都失败
# 高并发:秒杀，订单不并发处理的话，可能你一件商品能卖出来成千上万个订单，因为大家都在同一时间抢。
# 解决这个问题可以用悲观锁，执行的时候加琐 用户抢锁（就像多线程强一个全局变量）等先拿到锁的事务处理结束，琐释放，其他及用户才能拿到锁
# 或者用乐观锁：在查询数据时不加锁，在更新时进行判断，判断更新时的库存和之前查出的库存是否一致，开启乐观锁需要设在mysql事务的隔离性级别
class OrderCommitView(View):
    """订单创建"""
    # 事务装饰器
    @transaction.atomic # 包在一个sql事务,要么都成功要么都失败
    def post(self, request):
        """订单创建"""
        ...
        try:
            # todo： 向df_order_info（订单）表中添加一条记录
...
class OrderCommitView1(View):
    ...
    def post(self, request):
        """订单创建"""
        # 判断用户是否登录,后台无法使用LoginRequiredMixin验证,因为是ａｊａｘ提交过来
        user = request.user
        if not user.is_authenticated():
            # 用户美登录
            return JsonResponse({'res': 0, 'errmsg': '用户为登录'})
        ...
        try:
            ...
            for sku_id in sku_ids:
                for i in range(3):
                    ...
                    # where id=sku_id and stock = orgin_stock　　　这两句sql语句用django的M表示就是下面这一行
                    # 返回受影响的行数
...
class CheckPayView(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            return JsonResponse({'res': 2, 'errmsg': '订单错误'}) # 这些都是一样的
        ...
        # 使用python sdk调用支付宝交易查询接口,自己去网上找
        while True:
